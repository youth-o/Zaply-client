...
const ContentToggle = () => {
  const [activeTab, setActiveTab] = useState<"reserved" | "recent">("reserved");

  return (
    <div className="relative w-full bg-blue-300/30 rounded-[8px] py-[6px] px-2 flex items-center justify-between gap-2">
      <motion.div
        className="absolute top-[6px] left-[8px] h-[calc(100%-12px)] w-[calc(50%-8px)] rounded-[8px] bg-grayscale-100 z-0"
        layout
      ...
      <div className="relative z-10 w-1/2">
        <button
          ...
          예약된 콘텐츠 <span className="text-blue-700">0</span>
        </button>
      </div>
      <div className="relative z-10 w-1/2">
        <button
          ...
          최근 발행된 콘텐츠 <span className="text-blue-700">0</span>
        </button>
      </div>
    </div>
  );
